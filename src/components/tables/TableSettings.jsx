...
function SortableTableRow({ table }) {
  ...
  return (
    ...
    >
      ...
      <span
        {...attributes}
        {...listeners}
        style={{ cursor: 'grab', color: 'var(--silver)', fontSize: '0.9rem', flexShrink: 0, touchAction: 'none' }}
...
export default function TableSettings() {
  ...
  function handleDragEnd({ active, over }) {
    ...
    reorderTables(arrayMove(tables, oldIndex, newIndex))
  }
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={tables.map((t) => t.id)} strategy={verticalListSortingStrategy}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: 320, overflowY: 'auto' }}>
                {tables.map((table) => (
                  <SortableTableRow key={table.id} table={table} />
                ))}
              </div>
            </SortableContext>
          </DndContext>
